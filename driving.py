...
# def get_max_prob_num(prediction_array):
# 	"""将数组编码的整数转换为对应的数字"""
...
class ImageProcessor(threading.Thread):
	...
	def run(self):
		global model, latest_time, graph
		...
		while not self.terminated:
			...
			if self.event.wait(timeout=1):
				try:
					...
					image_up = np.array(image)
					camera_data_array = np.expand_dims(image_up, axis=0)
					...
					if current_time > latest_time:
						...
						with graph.as_default():
							prediction_array = model.predict(camera_data_array,batch_size=20, verbose=1)
						print(prediction_array)
						# action_num = get_max_prob_num(prediction_array)
						action_num = prediction_array.argmax()
						car_drive(action_num)
...
class ProcessOutput(object):
	...
	def __init__(self):
		super(ProcessOutput, self).__init__()
		self.done = False
...
def main():
	"""get data, then predict the data, edit data, then control the car"""
	global model, graph

	model_loaded = glob.glob('model/*.h5')
	for single_model in model_loaded:
